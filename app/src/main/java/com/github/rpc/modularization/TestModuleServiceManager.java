...
import com.github.rpc.module_login.LoginModule;
import com.github.rpc.module_login.LoginModuleServiceImpl;

import java.util.HashMap;
import java.util.Map;

public class TestModuleServiceManager {

    private static class RPCServiceManagerHolder {
        private static final TestModuleServiceManager INSTANCE = new TestModuleServiceManager();
    }

    public static TestModuleServiceManager getInstance() {
    ...
    private TestModuleServiceManager() {
    ...
    public static void initModules(Context context){
        initModule(context,new LoginModule());
    }
    ...
    /**
     * 方便的服务查找接口，不用先调用{@link TestModuleServiceManager#getInstance}取得实例，参数具体描述见{@link TestModuleServiceManager#findService}
     *
    ...
    public static <ModuleService extends Object> ModuleService findService(Class<ModuleService> service) {
        return TestModuleServiceManager.getInstance().innerFindService(service);
    }
